...
import { VulVulnerability } from './VulVulnerability';
...
export function App() {
  ...
  const [vulnerabilities, setVulnerabilities] = React.useState<VulVulnerability[]>([]);
  const [allVulnerabilities, setAllVulnerabilities] = React.useState<VulVulnerability[]>([]);
  ...
  async function createCacheVolume() {
    var success = true;
    await window.ddClient.docker.cli
      .exec("volume", ["create", "vul-docker-extension-cache"])
      .then((result: any) => {
        if (result.stderr !== "") {
          success = false;
        }
  ...
  async function triggerVul() {
    ...
    let commandParts: string[] = [
      ...
      "-v",
      "vul-docker-extension-cache:/root/.cache"
    ];

    if (uploadKhulnaSoft && !SBOMOutput) {
      commandParts.push("-e");
      commandParts.push("VUL_RUN_AS_PLUGIN=khulnasoft");
      commandParts.push("-e");
      commandParts.push("AQUA_KEY=" + khulnasoftKey);
      commandParts.push("-e");
      commandParts.push("AQUA_SECRET=" + khulnasoftSecret);
      commandParts.push("-e");
      commandParts.push("CSPM_URL=" + khulnasoftCSPMUrl);
    }


    commandParts.push("khulnasoftsec/vul");
    ...
    ({ stdout, stderr } = await runVul(commandParts, stdout, stderr));
  }

  async function runVul(commandParts: string[], stdout: string, stderr: string) {
    SendMetric("vul_scan_initiated", { imageName: scanImage });
        ...
        stream: {
          ...
          onClose(exitCode: number) {
            ...
            if (exitCode === 0) {
              ...
              SendMetric("vul_scan_succeeded", { imageName: scanImage });
              processResult(res);
            } else {
              SendMetric("vul_scan_failed", { imageName: scanImage });
              window.ddClient.desktopUI.toast.error(
  ...
  const processResult = (res: any) => {
    ...
    for (let i = 0; i < results.Results.length; i++) {
      ...
      for (let j = 0; j < r.Vulnerabilities.length; j++) {
        ...
        vulns.push(new VulVulnerability(v));
        all += 1;
        switch (v.Severity) {
          case "CRITICAL":
            critical += 1;
  ...
  React.useEffect(() => {
    ...
    SendMetric("vul_extension_opened", {});
  }, []);
